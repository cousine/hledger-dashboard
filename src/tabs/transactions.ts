import { DashboardContext, RegisterEntry } from '../hledger/types';
import { HledgerClient } from '../hledger/client';
import { buildTable, Column, Row } from '../ui/table';
...
export async function renderTransactions(
  ...
  function getSortKey(t: RegisterEntry, col: number): string | number {
    switch (col) {
      case 0: return t.date;
      case 1: return t.description;
      case 2: return t.account;
      case 3: return getTxnType(t.account, t.amount);
      case 4: return t.amount;
      default: return '';
    }
  }
  ...
  function renderContent() {
    ...
    if (ctx.filter.accountPatterns.length > 0) {
      filtered = filtered.filter(t => ctx.filter.accountPatterns.some(p => matchPattern(t.account, p)));
    }
    ...
    if (txnTypeFilter !== 'all') {
      filtered = filtered.filter(t => getTxnType(t.account, t.amount).toLowerCase() === txnTypeFilter);
    }
    ...
    buildKpiCard(kpiRow, 'Inflow', formatAmount(totalInflow, ctx.targetCurrency), 'hldg-value-positive');
    buildKpiCard(kpiRow, 'Outflow', formatAmount(totalOutflow, ctx.targetCurrency), 'hldg-value-negative');
    buildKpiCard(kpiRow, 'Net', formatAmount(net, ctx.targetCurrency), net >= 0 ? 'hldg-value-positive' : 'hldg-value-negative');
    ...
    tier2Btn.addEventListener('click', () => {
      if (breakdownDepth !== 2) { breakdownDepth = 2; ctx.onUIStateChange?.({ breakdownDepth: 2 }); renderContent(); }
    });
    ...
    tier3Btn.addEventListener('click', () => {
      if (breakdownDepth !== 3) { breakdownDepth = 3; ctx.onUIStateChange?.({ breakdownDepth: 3 }); renderContent(); }
    });
    ...
    if (expBreakdown.labels.length > 0 || incBreakdown.labels.length > 0 || liabBreakdown.labels.length > 0) {
      ...
      if (expBreakdown.labels.length > 0) {
        ...
        createDoughnutChart(chartCol, expBreakdown.labels, expBreakdown.data,
      ...
      if (incBreakdown.labels.length > 0) {
        ...
        createDoughnutChart(chartCol, incBreakdown.labels, incBreakdown.data,
      ...
      if (liabBreakdown.labels.length > 0) {
        ...
        createDoughnutChart(chartCol, liabBreakdown.labels, liabBreakdown.data,
    ...
    allBtn.addEventListener('click', () => {
      if (txnTypeFilter !== 'all') { txnTypeFilter = 'all'; ctx.onUIStateChange?.({ txnTypeFilter: 'all' }); renderContent(); }
    });
    ...
    creditBtn.addEventListener('click', () => {
      if (txnTypeFilter !== 'credit') { txnTypeFilter = 'credit'; ctx.onUIStateChange?.({ txnTypeFilter: 'credit' }); renderContent(); }
    });
    ...
    debitBtn.addEventListener('click', () => {
      if (txnTypeFilter !== 'debit') { txnTypeFilter = 'debit'; ctx.onUIStateChange?.({ txnTypeFilter: 'debit' }); renderContent(); }
    });
    ...
    if (totalPages > 1) {
      ...
      firstBtn.addEventListener('click', () => { currentPage = 0; renderContent(); });

      const prevBtn = nav.createEl('button', { cls: 'hldg-page-btn', text: '‹ Prev' });
      prevBtn.disabled = currentPage === 0;
      prevBtn.addEventListener('click', () => {
        if (currentPage > 0) { currentPage--; renderContent(); }
      });

      nav.createSpan({ cls: 'hldg-page-info', text: `Page ${currentPage + 1} of ${totalPages} (${filtered.length} transactions)` });

      const nextBtn = nav.createEl('button', { cls: 'hldg-page-btn', text: 'Next ›' });
      nextBtn.disabled = currentPage >= totalPages - 1;
      nextBtn.addEventListener('click', () => {
        if (currentPage < totalPages - 1) { currentPage++; renderContent(); }
      });

      const lastBtn = nav.createEl('button', { cls: 'hldg-page-btn', text: 'Last >>' });
      lastBtn.disabled = currentPage >= totalPages - 1;
      lastBtn.addEventListener('click', () => { currentPage = totalPages - 1; renderContent(); });
    }
