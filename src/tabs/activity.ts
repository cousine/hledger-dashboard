...
export async function renderActivity(
  ...
  const [stdoutX, stdoutNative] = await Promise.all([
    client.exec(ctx.settings.hledgerPath, ['-X', ctx.targetCurrency, ...baseArgs], ctx.settings.journalFile),
  ...
  buildKpiCard(kpiRow, 'Income', formatAmount(incomeTotal, ctx.targetCurrency), 'hldg-value-positive');
  buildKpiCard(kpiRow, 'Expenses', formatAmount(expenseTotal, ctx.targetCurrency), 'hldg-value-negative');
  buildKpiCard(kpiRow, 'Net', formatAmount(netTotal, ctx.targetCurrency), netTotal >= 0 ? 'hldg-value-positive' : 'hldg-value-negative');

  container.createEl('br');

  const expenseChildrenX = filteredX.filter(e => e.depth === 1 && e.account.startsWith('expenses:'));
  const expenseAtomicX = filteredX.filter(e => e.depth === 2 && e.account.startsWith('expenses:'));
  const expenseChildrenNative = filteredNative.filter(e => e.depth === 1 && e.account.startsWith('expenses:'));
  const expenseAtomicNative = filteredNative.filter(e => e.depth === 2 && e.account.startsWith('expenses:'));
  const incomeChildrenNative = filteredNative.filter(e => e.depth === 1 && e.account.startsWith('income:'));
  ...
  if (isMultiMonth && (incomeChildrenNative.length > 0 || filteredX.some(e => e.depth === 0 && e.account.startsWith('income')))) {
    try {
  ...
  function renderExpenseSection() {
    ...
    if (showNative.length > 0) {
      const rows: Row[] = showNative.map(e => {
        ...
        return [
          { text: name, cls: 'hldg-clickable-account', onClick: () => ctx.onNavigate?.('transactions', [fullAccount]) },
          { text: formatAmount(e.amount, e.commodity), cls: '', sortValue: e.amount },
          { text: pct, sortValue: pctVal },
        ];
      });
      rows.sort((a, b) => {
        const av = (a[2] as any)?.sortValue ?? 0;
        const bv = (b[2] as any)?.sortValue ?? 0;
        return bv - av;
      });
      createPaginatedTable(
        tableCol,
        [{ label: catLabel }, { label: 'Amount', align: 'right', width: '160px' }, { label: '%', align: 'right', width: '60px' }],
        rows,
